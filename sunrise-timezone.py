...
def main() -> None:
    ...
    for day, sunrise in sunrise_calculator(2013, latitude, longitude, sunrise_set_noon):
        if (
            day.tm_yday % 2 == 0
        ):  # needs to skip every other day otherwise there are too many lines (limit is 256) https://codebrowser.dev/glibc/glibc/timezone/zic.c.html#2987 -> https://codebrowser.dev/glibc/glibc/timezone/tzfile.h.html#111
            continue
        #            Rule  NAME  FROM  TO  TYPE  IN  ON  AT  SAVE  LETTER/S
        file.write(
            f'Rule Rule {start_year} {end_year} - {months[day.tm_mon]} {day.tm_mday:2} {int((-sunrise+recalibration_time)//60)+offset:2}:{int((sunrise+recalibration_time)%60):02} {-int(sunrise//60)+offset:2}:{int(sunrise%60):02}:{int(sunrise%1*60):02} - \n'
        )
...
def sunrise_calculator(
    year: int, latitude_deg: float, longitude_deg: float, sunrise: str = 'r'
) -> list[tuple[time.struct_time, float]]:  # returns sunrises of the year in utc
    ...
    def calculator(day: int) -> float:
        ...
        hour_angle = acos(
            cos(1.58533492) / (cos(latitude) * cos(declination)) - tan(latitude) * tan(declination)
        )  # in rad. 1.58533492rad = 90.833deg
